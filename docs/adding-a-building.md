...
`check.mjs` asserts what is true of every building — finite geometry inside a
believable box, real stock lengths, share codes that round-trip — and then a
building-specific block. Extend that block; a check that cannot fail is not
worth the runtime.
